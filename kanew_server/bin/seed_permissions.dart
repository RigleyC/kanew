...
class PermissionService {
  ...
  /// Call this manually to initialize permissions if serverpod generate is not available
  /// Usage:
  /// ```dart
  /// import 'package:serverpod/serverpod.dart';
  /// import 'package:kanew_server/lib/src/services/permission_service.dart';
  ///
  /// void main() async {
  ///   final pod = Serverpod()..connectToLocalDatabase();
  ///   await PermissionService.seedPermissions(pod.createSession());
  /// }
  /// ```
  ...
  static Future<List<String>> getUserPermissions(
    ...
    final members = await MemberPermission.db.find(
      session,
      where: (t) =>
          t.workspaceMemberId.workspaceId.equals(workspaceId) &
          t.workspaceMemberId.userInfoId.equals(userId) &
          t.workspaceMemberId.deletedAt.equals(null),
    );

    final permissionIds = members.map((mp) => mp.permissionId).toSet();

    final permissions = await Permission.db.find(
      session,
      where: (p) => p.id.inList(permissionIds.toList()),
    );
